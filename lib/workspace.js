...
Workspace.prototype._format_state = function() {
  ...
  for (var name in this._state) {
    var obj = {
      name: name,
      state: this._state[name]["state"]
    };

    if (this._state[name]["interfaces"]) {
      obj["interfaces"] = [];
      for (var iface in this._state[name]["interfaces"])
        obj["interfaces"].push({
          interface: iface,
          ip: this._state[name]["interfaces"][iface]
        });
    }
...
Workspace.prototype._update_state = function(nodes) {
  ...
  for (var i = 0; i < nodes.length; i++) {
    ...
    if (!this._state[nodes[i].name]["interfaces"])
